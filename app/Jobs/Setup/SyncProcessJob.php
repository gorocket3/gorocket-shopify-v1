...
class SyncProcessJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$shop) {
                Log::error("[SYNC][PRODUCT] Shop not found - {$this->shopId}");
                return;
            }
            ...
            if($totalProducts > 0) {
                Redis::hset("shop:{$this->shopId}:product_sync", 'syncing', true);
                Redis::hset("shop:{$this->shopId}:product_sync", 'progress', 0);
                Redis::hset("shop:{$this->shopId}:product_sync", 'bulking', 0);
                Redis::expire("shop:{$this->shopId}:product_sync", 1800);
            } else {
            ...
            $this->deleteDBProducts($this->shopId, $batchSize);
            ...
            do {
                $response = retry(3, function () use ($shop, $chunk, $nextPage) {
                    ...
                    Log::warning("[SYNC][PRODUCT] API retry failed: {$throwable->getMessage()}");
                    return 1000;
                });

                if (($response['errors'] ?? false) || !isset($response['body']['products'])) {
                    Log::error("[SYNC][PRODUCT] API failed - {$this->shopId}");
            ...
            if (!empty($batch)) {
                ProductUpdateJob::dispatch($batch, $this->shopId, 100);
            }
            Log::info("[SYNC][PRODUCT] Queue success - {$this->shopId}");

            $this->runBulkProductGraphQL($shop, $chunk, $totalProducts);

        } catch (Exception $e) {
            Log::error("[SYNC][PRODUCT] Queue failed - {$this->shopId}, Error: {$e->getMessage()}");
            Redis::del("shop:{$this->shopId}:product_sync");
        }
    }
    ...
    private function runBulkProductGraphQL(User $shop, int $chunk, int $totalProducts): void
    {
        ...
        if ($errors) {
            Log::error("[SYNC][BULK] Bulk failed - {$shop->id}");
        } else {
            Redis::hset("shop:{$shop->id}:product_sync", 'bulking', true);
            Log::info("[SYNC][BULK] Bulk initiated - {$shop->id}");
        }
    }
}
